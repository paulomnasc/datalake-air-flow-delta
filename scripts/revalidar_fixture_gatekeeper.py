...
def audit_ah_lines_reasons(betano_lines, status_ah, sug_ah, fix, h_trend, a_trend):
    """
...
def inject_audit_into_compound_reasoning(compound_r, audit_bullet):
    ...
    if "|| MOTIVACAO:" in compound_r and "|| MEMÓRIA" in compound_r:
        ...
        # Remove versão anterior de auditoria de linhas se já existia
        if "• 📋 Linhas de Handicap Asiático Auditadas" in mot_part:
            mot_part = mot_part.split("• 📋 Linhas de Handicap Asiático Auditadas")[0].strip()

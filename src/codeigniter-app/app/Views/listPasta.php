...
?>
        <div id="content">        
            <div class="container">
            <h4 style="text-align: center;">Listagem de Pastas</h4>

                <input type="text" id="filtro-nome" placeholder="Filtrar por descrição">
            <img src="../assets/img/lupa.jpg" >
            
            <form action="<?php echo site_url('addPasta'); ?>" method="post">
                ...
                <table class="data-table" id="data-table">
                    ...
                    <tbody>
                        ...
                        <?php foreach($list as $Pasta): ?>
                        <tr>
                            <td> <?php echo $Pasta->id ?> </td>
                            <td> <?php echo $Pasta->descricao ?> </td>
                            <td> 
                                <div class="sidebyside-container">
                                    <form action="<?php echo site_url('updPasta'); ?>" method="post">
                                        <input type="hidden" name="id" id="id" value="<?php echo $Pasta->id ?>">
                                        <button class="edit-button" onclick="">✏️</button>
                                    </form>
                                    <form id="deleteForm-<?php echo $Pasta->id; ?>">
                                        <button class="delete-button" type="button" onclick="confirmDelete('<?php echo $Pasta->id; ?>', '<?php echo $Pasta->descricao; ?>', '<?php echo site_url('deletePasta/' . $Pasta->id); ?>', 'deleteForm-<?php echo $Pasta->id; ?>')">🗑️</button>
                                    </form>
                                </div>

                                <script>
                                    function confirmDelete(id, descricao, deleteUrl, formId) {
                                        if (confirm("Você tem certeza que deseja deletar este Pasta: " + id + " - " + descricao + "?")) {
                                            $.ajax({
                                                url: deleteUrl,
                                                type: 'POST',
                                                data: {
                                                ...
                                                success: function(result) {
                                                    ...
                                                    $('#main-content').load('<?php echo route_to('listPasta'); ?> #main-content');
                                                },
                                                error: function(err) {
                                                    $('#error-message').html('Erro ao excluir o registro.').show().delay(6000).fadeOut(); // Mostra a mensagem de erro

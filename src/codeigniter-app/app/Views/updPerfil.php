...
<div id="main-content">
    ...
    <div id="content">
        ...
        <div class="container">
            <h1>Editar Perfil</h1>
            <!-- updPerfil.php -->
            <form method="post" id="meuFormulario" action="<?php echo route_to('Perfil.update'); ?>">
                    ...
                    <select id="id_funcionalidade" name="id_funcionalidade[]" multiple style="height: 120px;">
                        <?php if(isset($funcionalidades) && !empty($funcionalidades)): ?>
                            <?php foreach($funcionalidades as $funcionalidade): ?>
                                <option value="<?php echo $funcionalidade->id; ?>" <?php echo (in_array($funcionalidade->id, $funcionalidades_selecionadas)) ? 'selected' : ''; ?>>
            ...
            $('#meuFormulario').submit(function(event) {
                ...
                $.ajax({
                    ...
                    error: function(err) {
                        $('#error-message').html('Erro ao atualizar o perfil.').show().delay(6000).fadeOut(); // Mostra a mensagem de erro

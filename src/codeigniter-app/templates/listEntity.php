...
<div id="main-content">
        ...
        <div id="content">        
            ...
            <form action="<?php echo site_url('add<%entity%>'); ?>" method="post">
                ...
                <table class="data-table" id="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td> <?php echo $<%entity%>->descricao ?> </td>
                            <td> 
                                <div class="sidebyside-container">
                                    <form action="<?php echo site_url('upd<%entity%>'); ?>" method="post">
                                        <input type="hidden" name="id" id="id" value="<?php echo $<%entity%>->id ?>">
                                        <button class="edit-button" onclick="">✏️</button>
                                    </form>
                                    <form id="deleteForm-<?php echo $<%entity%>->id; ?>">
                                        <button class="delete-button" type="button" onclick="confirmDelete('<?php echo $<%entity%>->id; ?>', '<?php echo $<%entity%>->descricao; ?>', '<?php echo site_url('delete<%entity%>/' . $<%entity%>->id); ?>', 'deleteForm-<?php echo $<%entity%>->id; ?>')">🗑️</button>
                                    </form>
                                </div>

                                <script>
                                    function confirmDelete(id, descricao, deleteUrl, formId) {
                                        if (confirm("Você tem certeza que deseja deletar este <%entity%>: " + id + " - " + descricao + "?")) {
                                            $.ajax({
                                                url: deleteUrl,
                                                type: 'POST',
                                                data: {
                                                ...
                                                success: function(result) {
                                                    ...
                                                    $('#main-content').load('<?php echo route_to('list<%entity%>'); ?> #main-content');
                                                },
                                                error: function(err) {
                                                    $('#error-message').html('Erro ao excluir o registro.').show().delay(6000).fadeOut(); // Mostra a mensagem de erro

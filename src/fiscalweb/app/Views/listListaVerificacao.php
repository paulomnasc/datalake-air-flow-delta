...
<div id="content">        
    <div class="container">
        <h4 style="text-align: center;">Listagem de Lista de Verificação (Checklist)</h4>
        
        <input type="text" id="filtro-descricao" placeholder="Filtrar">
        <img src="../assets/img/lupa.jpg" >
        
        <form action="<?php echo site_url('addListaVerificacao'); ?>" method="post">
                ...
                <tr id="row-<?php echo $item->id ?>">
                    ...
                    <td> 
                        <div class="sidebyside-container">
                            <form action="<?php echo site_url('updListaVerificacao'); ?>" method="post">
                                <input type="hidden" name="id" value="<?php echo $item->id ?>">
                                <button class="edit-button" type="submit">✏️</button>
                            </form>
                            <form id="deleteForm-<?php echo $item->id; ?>">
                                <button class="delete-button" type="button" onclick="confirmDelete('<?php echo $item->id; ?>', '<?php echo site_url('deleteListaVerificacao/' . $item->id); ?>', 'deleteForm-<?php echo $item->id; ?>')">🗑️</button>
                            </form>
                        </div>
                    </td>
                </tr>
        ...
        <script>
            function confirmDelete(id, deleteUrl, formId) {
                if (confirm("Você tem certeza que deseja deletar este registro de checklist?")) {
                    $.ajax({
                        url: deleteUrl,
                        type: 'POST',
                        data: { _method: 'DELETE' },

...
class AirflowHelper
{
    ...
    /**
     * Dispara a execução de qualquer DAG no Airflow com parâmetros de configuração
     * 
     * @param string $dagId ID da DAG
     * @param array $conf Configurações a serem passadas à DAG (key => value)
     * @return array ['success' => bool, 'message' => string, 'run_id' => ?string, 'error' => ?string]
     */
    public static function triggerDag(string $dagId, array $conf = []): array
    {
        try {
            log_message('info', "[AirflowHelper] Disparando DAG '{$dagId}' com config: " . json_encode($conf));
            
            $airflowUrl = self::getAirflowBaseUrl() . "/api/v1/dags/{$dagId}/dagRuns";
            ...
            if ($result['success']) {
                $runId = $result['response']['dag_run_id'] ?? null;
                log_message('info', "[AirflowHelper] ✅ DAG '{$dagId}' disparada com sucesso! Run ID: {$runId}");
                return [
                    'success' => true,
                    'message' => "DAG '{$dagId}' disparada com sucesso",
                    'run_id' => $runId
                ];
            } else {
                log_message('error', "[AirflowHelper] Falha ao disparar DAG '{$dagId}': " . ($result['error'] ?? 'Erro desconhecido'));
                return [
                    'success' => false,
                    'message' => "Falha ao disparar DAG no Airflow: " . ($result['error'] ?? 'Erro desconhecido'),
                    'error' => $result['error'] ?? 'Erro desconhecido'
                ];
            }
        } catch (\Exception $e) {
            log_message('error', "[AirflowHelper] Exception ao disparar DAG '{$dagId}': " . $e->getMessage());
            return [
                'success' => false,
                'message' => "Erro de exceção ao disparar DAG: " . $e->getMessage(),
                'error' => $e->getMessage()
            ];
        }
    }

...
<div id="content">        
    <div class="container">
        <h4 style="text-align: center;">Listagem de AreaAtuacao</h4>
        
        <input type="text" id="filtro-id_catalogo_servicos" placeholder="Filtrar">
        <img src="../assets/img/lupa.jpg" >
        
        <form action="<?php echo site_url('addAreaAtuacao'); ?>" method="post">
        ...
        <table class="data-table" id="data-table">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>IdCatalogoServicos</th><th>Descricao</th>
                    <th>Ações</th>
                </tr>
            </thead>
            <tbody>
            ...
            <td>
                <select name="id_catalogo_servicos" id="id_catalogo_servicos-<?php echo $item->id ?>">
                    <option value="">Selecione...</option>
                    <?php if(isset($id_catalogo_servicos_list)): foreach($id_catalogo_servicos_list as $opt): ?>
                        <option value="<?php echo $opt->id; ?>" <?php if($opt->id == $item->id_catalogo_servicos) echo 'selected'; ?>>
...
<td> <?php echo $item->descricao ?> </td>
                    <td> 
                        <div class="sidebyside-container">
                            <form action="<?php echo site_url('updAreaAtuacao'); ?>" method="post">
                                <input type="hidden" name="id" value="<?php echo $item->id ?>">
                                <button class="edit-button" type="submit">✏️</button>
                            </form>
                            <form id="deleteForm-<?php echo $item->id; ?>">
                                <button class="delete-button" type="button" onclick="confirmDelete('<?php echo $item->id; ?>', '<?php echo site_url('deleteAreaAtuacao/' . $item->id); ?>', 'deleteForm-<?php echo $item->id; ?>')">🗑️</button>
                            </form>
                        </div>
                    </td>
                </tr>

...
?>
        <div id="content">        
            <div class="container">
            <h4 style="text-align: center;">Listagem de Perfis</h4>

                <input type="text" id="filtro-nome" placeholder="Filtrar por descrição">
            <img src="../assets/img/lupa.jpg" >
            
            <form action="<?php echo site_url('addPerfil'); ?>" method="post">
                ...
                <table class="data-table" id="data-table">
                    ...
                    <tbody>
                        ...
                        <?php foreach($list as $perfil): ?>
                        <tr>
                            <td> <?php echo $perfil->id ?> </td>
                            <td> <?php echo $perfil->descricao ?> </td>
                            <td> 
                                <div class="sidebyside-container">
                                    <form action="<?php echo site_url('updPerfil'); ?>" method="post">
                                        <input type="hidden" name="id" id="id" value="<?php echo $perfil->id ?>">
                                        <button class="edit-button" onclick="">✏️</button>
                                    </form>
                                    <form id="deleteForm-<?php echo $perfil->id; ?>">
                                        <button class="delete-button" type="button" onclick="confirmDelete('<?php echo $perfil->id; ?>', '<?php echo $perfil->descricao; ?>', '<?php echo site_url('deletePerfil/' . $perfil->id); ?>', 'deleteForm-<?php echo $perfil->id; ?>')">🗑️</button>
                                    </form>
                                </div>

                                <script>
                                    function confirmDelete(id, descricao, deleteUrl, formId) {
                                        if (confirm("Você tem certeza que deseja deletar este perfil: " + id + " - " + descricao + "?")) {
                                            $.ajax({
                                                url: deleteUrl,
                                                type: 'POST',
                                                data: {
                                                ...
                                                success: function(result) {
                                                    ...
                                                    $('#main-content').load('<?php echo route_to('listPerfil'); ?> #main-content');
                                                },
                                                error: function(err) {
                                                    $('#error-message').html('Erro ao excluir o registro.').show().delay(6000).fadeOut(); // Mostra a mensagem de erro
